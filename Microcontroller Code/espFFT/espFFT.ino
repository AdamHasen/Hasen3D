...
#define SAMPLING_FREQ   24000         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
...
ArduinoFFT<float> FFT = ArduinoFFT<float>(vReal, vImag, SAMPLES, SAMPLING_FREQ, true);
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    ...
    //8 bands, 12kHz top band
      if (i<=3 )           bandValues[7]  += (int)vReal[i];
      if (i>3   && i<=6  ) bandValues[6]  += (int)vReal[i];
      if (i>6   && i<=13 ) bandValues[5]  += (int)vReal[i];
      if (i>13  && i<=27 ) bandValues[4]  += (int)vReal[i];
      if (i>27  && i<=55 ) bandValues[3]  += (int)vReal[i];
      if (i>55  && i<=112) bandValues[2]  += (int)vReal[i];
      if (i>112 && i<=229) bandValues[1]  += (int)vReal[i];
      if (i>229          ) bandValues[0]  += (int)vReal[i];
